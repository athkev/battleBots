	...
	for (int i = 0; i < n_curves; i++)
	{
		for (int j = 0; j < subcurve.numCurvePoints - 1; j++)
		{
			if (i == n_curves - 1) {
				...
				glTexCoord2f(0.5 + vertexArray[i][j].x /4 * 0.5, 0.5 + vertexArray[i][j].y /4 * 0.5);
...
//////////////////////////////////////////////////////////
GLuint turtleTexture = 0;//textures
GLuint texture0;
...
int main(int argc, char** argv)
{
	...
	turtleTexture = LoadTexture("../data/texture/shell.dds");
	...
	int image_width = 0;
	int image_height = 0;
	unsigned char* image = SOIL_load_image("shell.jpg", &image_width, &image_height, NULL, SOIL_LOAD_RGBA);
...
void display(void)
{
	...
	glScalef(bodySizex, bodySizey, bodySizez);
	glutSolidCube(1);
	glPopMatrix();
...
void keyboard(unsigned char key, int x, int y)
{
		if (key == 'e') {
			...
			tempX = X;
			tempZ = Z;
			X -= 1 * sinf(-bodyAngle*2.0 * PI / 180);
			Y += 0;
			Z += 1 * cosf(-bodyAngle*2.0 * PI / 180);
			moving = true;
			turning = false;
			wheelAngleL += 20;
			wheelAngleR += 20;
